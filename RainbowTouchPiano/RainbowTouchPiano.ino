...
void setup() {
  ...
  midiPlayer.midiSetInstrument(0, 2);
  delay(200);

  /**********彩灯与音乐逐渐点亮与响起*/
  for (int a = 0; a < 7; a++) {
    midiPlayer.noteOn(0, toneNum[a], 120);     //播放相应音级
    strip.setPixelColor(2*a, colorArray[a + 1]); //设置相应灯的颜色
    strip.setPixelColor(2*a+1, colorArray[a + 1]); //设置相应灯的颜色
    strip.show();                              //将灯点亮成新的颜色
    delay(100);
    midiPlayer.noteOff(0, toneNum[a], 120);    //关闭相应音级
    midiPlayer.noteOff(1, toneNum[a], 120);     //播放相应音级
    strip.setPixelColor(2*a, colorArray[0]); //设置相应灯的颜色
    strip.setPixelColor(2*a+1, colorArray[0]); //设置相应灯的颜色
    strip.show();                              //将灯点亮成新的颜色
  }
  delay(500);
